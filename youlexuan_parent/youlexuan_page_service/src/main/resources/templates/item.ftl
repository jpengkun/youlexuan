							...
							<#list specificationList as spec>
								<dl>
									...
									<#list spec.attributeValue as item>
										<dd ng-click="selectSpecification('${spec.attributeName}','${item}')"><a href="javascript:void(0);" class="{{isSelected('${spec.attributeName}','${item}')?'selected':''}}">${item}<span title="点击取消选择">&nbsp;</span></a></dd>
									</#list>
								</dl>
							</#list>
						</div>
